...
const addToCartController = async function(productID) {
    try {
        if (!productID) {
            if (_modelJs.state.cart.length > 0) {
                (0, _cartViewJsDefault.default).updateCartSubheading("View the items in your cart");
                return (0, _cartViewJsDefault.default).render(_modelJs.state.cart);
            }
            return (0, _cartViewJsDefault.default).updateCartSubheading();
        }
        const product = await _modelJs.loadProduct(productID);
        const productExistsInCart = _modelJs.state.cart.find((el)=>el.id === product.id);
        if (productExistsInCart) return;
        (0, _cartViewJsDefault.default).renderSpinner();
        _modelJs.addToCart(product);
...
const deleteCartItemController = function(productID) {
    _modelJs.deleteCartItem(productID);
    (0, _cartViewJsDefault.default).renderSpinner();
    if (!_modelJs.state.cart.length) {
        (0, _cartViewJsDefault.default).clear();
...
const cartItemClickedController = async function(productID) {
    (0, _mainNavigationJsDefault.default).toggleCart();
...
const initate = async function() {
    try {
        ...
        (0, _cartViewJsDefault.default).addHandlerAddToCartClicked(addToCartController);
        (0, _cartViewJsDefault.default).addHandlerCartItemClicked(cartItemClickedController);
        (0, _cartViewJsDefault.default).addHandlerCartItemDeleted(deleteCartItemController);
    } catch (error) {
        console.log(error);
    }
}();
...
parcelHelpers.export(exports, "loadCart", ()=>loadCart);
...
const deleteCartItem = function(productID) {
    const newCart = state.cart.filter((element)=>element.id !== Number(productID));
    state.cart = newCart;
    saveCart();
};
...
class mainNavigation extends (0, _viewDefault.default) {
    ...
    #handleMenusClick() {
        window.addEventListener("click", (e)=>{
            ...
            if (target.closest('[data-btn-name="menu-btn"]') || this.menuOpen) return this.toggleMenu();
            if (target.closest('[data-btn-name="cart-btn"]') || this.cartOpen) return this.toggleCart();
        });
    }
    toggleCart() {
        ...
        this.cartOpen = this.cartOpen ? this.cartOpen = false : this.cartOpen = true;
    ...
    toggleMenu() {
        ...
        const overlay = document.querySelector(".overlay");
        const mainElement = document.querySelector("main");
        this.menuOpen = this.menuOpen ? this.menuOpen = false : this.menuOpen = true;
    ...
    #changeCartIcon() {
        ...
        this.cartOpen ? cartIcon.classList.add("cart-fill") : cartIcon.classList.remove("cart-fill");
    }
}
...
class CartView extends (0, _viewDefault.default) {
    ...
    addHandlerCartItemClicked(func) {
        this.parentElement.addEventListener("click", function(e) {
            ...
            if (!target || e.target.closest('[data-btn-name="clear-cart-item"]')) return;
            func(target.dataset.id);
        });
    }
    ...
    generateMarkup(data) {
        const markup = data.map((product)=>{
            return `<a href="#${product.id}" class="link"><li class="cart-item" data-id=${product.id}>
      <span
    ...
    >
      ...
      <div class="cart-image-container">
        <img src="${product.image}" alt="${product.description}" class="cart-image" />
      </div>
      <div class="cart-item-details">
        ...
        <div class="cart-item-attributes">
          ...
          <p class="cart-item-rating">${"⭐".repeat(Math.round(product.rating.rate))}</p>
        </div>
      </div>
    </li></a>`;
        });
